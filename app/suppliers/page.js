...
export default function SuppliersPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      if (editId) {
        await fetch(`/api/suppliers/${editId}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify(form) });
        setEditId(null);
  ...
  async function confirmDelete() {
    try {
      await fetch(`/api/suppliers/${confirm.id}`, { method: "DELETE" });
  ...
  return (
    <div>
      ...
      <h1 style={{ fontSize: "24px", fontWeight: "800", color: "#09637E", marginBottom: "8px" }}>Suppliers</h1>
      <p style={{ color: "black", marginBottom: "28px", fontSize: "14px" }}>Manage your supplier information</p>
      ...
      <div style={{ background: "white", borderRadius: "16px", padding: "24px", marginBottom: "24px", boxShadow: "0 2px 12px rgba(9,99,126,0.08)", borderTop: "4px solid #088395" }}>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "12px" }}>
            <input required placeholder="Supplier Name" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} style={inputStyle} />
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            ...
            {editId && (
              <button type="button" onClick={() => { setEditId(null); setForm({ name: "", email: "", phone: "", address: "" }); }} style={{ background: "#e0e0e0", color: "#555", border: "none", borderRadius: "8px", padding: "10px 16px", fontWeight: "600", cursor: "pointer" }}>
                Cancel
              </button>
            )}
          </div>
      ...
      <div style={{ background: "white", borderRadius: "16px", padding: "24px", boxShadow: "0 2px 12px rgba(9,99,126,0.08)" }}>
        <div className="flex flex-wrap justify-between items-center gap-3 mb-4">
          <h2 style={{ fontWeight: "700", color: "#09637E" }}>All Suppliers ({filtered.length})</h2>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "14px" }}>
            ...
            <tbody>
              {loading ? (
                ...
                <EmptyState colSpan={5} message={search ? `No suppliers matching "${search}"` : "No suppliers yet. Add your first supplier!"} />
              ) : (
                paginated.map((s) => (
                  <tr key={s.id} style={{ borderBottom: "1px solid #f0f7f8" }}>
                    <td style={{ padding: "12px 14px", fontWeight: "600", color: "#09637E" }}>{s.name}</td>
                    ...
                    <td style={{ padding: "12px 14px" }}>
                      <button onClick={() => handleEdit(s)} style={{ background: "#fff3e0", color: "#e65100", border: "none", borderRadius: "6px", padding: "5px 12px", cursor: "pointer", fontSize: "12px", fontWeight: "600", marginRight: "6px" }}>Edit</button>
