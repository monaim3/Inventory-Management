...
async function getDashboardData() {
  ...
  const [products, reports, categories, suppliers] = await Promise.all([
    ...
    fetch(`${base}/api/categories`, { cache: "no-store" }).then((r) =>
      r.json(),
    ),
...
export default async function Dashboard() {
  ...
  return (
    <div>
      ...
      <p className="text-[#7AB2B2] text-sm mb-7">Overview of your inventory</p>
      ...
      {reports.lowStock?.length > 0 && (
        <div className="bg-[linear-gradient(135deg,#fff5f5,#ffe8e8)] border border-[#ffcdd2] rounded-2xl px-6 py-5 mb-7">
          <h2 className="text-[#e53935] font-bold mb-2.5">
            ⚠️ Low Stock Alert
          </h2>
        ...
        <table className="w-full border-collapse text-sm">
          <thead>
            <tr className="bg-[#EBF4F6]">
              {["Name", "Category", "Supplier", "Price", "Quantity"].map(
                (h) => (
                  <th
                    key={h}
